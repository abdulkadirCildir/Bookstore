...
const BookList = () => {
  ...
  useEffect(() => {
    const newList = selectedTag === "Any" ? bookList : bookList.filter((item) => {
          ...
          ? filteredBookList.map((book, index) => {
              return (
                <MediaCard
                  ...
                  // imgSrc={book?.category ? `http://placeimg.com/140/200/${
                  //   book?.category}` : `https://cdn.preis.de/p/1324205/225x225/85/1/no-image.jpg`
                  // }
                />
              );
            })
